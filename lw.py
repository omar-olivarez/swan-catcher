...
class Character(object):
    def __init__(self, name, corpus,age, race):
        self.name = name
        self.corpus = corpus
        ...
        for character in self.clist:
            ...
            for word in character.word_set:
                if word not in self.word_usage.keys():
                    ...
                    self.word_usage[word] += 1
        self.cutoff = len(self.clist)//20 # words must be used by less than 5% of the population
        ...
        for i in range(len(newClist)):
            ...
            if char.rank==-1:
                ...
                for j in range(numtimes): # for every time that this amount of per_special appears
                    ...
                    if len(c.total_word_counts) > 25 and c.special_word_count > 5: #if that character says more than 30 different words and says a special word at least 5 times
...
def order(characters):
    ...
    perc = round(perc, 2)
    st = "The %d most different people on deathrow (top %.2f percent)" %(characters.special_char, perc)
    print(st)
    output.write(st)
    output.write("\n")
    for i in range(1,characters.special_char+1):
        for character in characters.clist:
            if character.rank == i:
                ...
                print(st)
                output.write(st)
                stt = "Age: %4s  Race: %10s" %(character.age, character.race)
                print(stt)
                output.write(stt)
                print()
                output.write("\n\n")
...
def main():
    corpus = pd.read_csv("lastwords.csv", delimiter=",")
    character_col = 0
    line_col = 8
    age_col = 4
    race_col = 6
    ...
    for i in range(1,total_rows+1):
        ...
        if not characters.find(character): # returns true if character is found
            ...
            text += " "
            agee = corpus.iloc[rownum, age_col]
            racee = corpus.iloc[rownum, race_col]
            new_character = Character(character, text, agee, racee)
