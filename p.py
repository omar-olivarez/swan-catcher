...
class CharacterList(object):
    def __init__(self):
        ...
        self.special_char = 0 # this is the total number of characters that are special
    def __str__(self):
        ...
        for character in self.clist:
            ...
            for word in character.word_set:
                if word not in self.word_usage.keys():
                    ...
                    self.word_usage[word] += 1
        self.cutoff = len(self.clist)//33 # words must be used by less than 3% of the population, in other words these are words say only by that president
    ...
    def rank_characters(self):
        ...
        for i in range(len(newClist)):
            ...
            if char.rank==-1:
                ...
                for j in range(numtimes): # for every time that this amount of per_special appears
                    ...
                    if len(c.total_word_counts) > 25 and c.special_word_count > 20: #if that character says more than 30 different words and says a special word at least 5 times
        ...
        # take 5% of that
        cutoffrank = maxrank // 10 # take the top 20% ranking special characters
...
def order(characters):
    output = open("most_different_presidents_output.txt", 'w')
    perc = characters.special_char / len(characters.clist) *100
    perc = round(perc, 2)
    print("Total presidents:", len(characters.clist))
    st = "The %d most different presidents (top %.2f percent)" %(characters.special_char, perc)
    print(st)
    output.write(st)
    output.write("\n")
    for i in range(1,characters.special_char+1):
        for character in characters.clist:
            if character.rank == i:
                ...
                print("---------------------------------------------------------------------------------------------------------")
                output.write("---------------------------------------------------------------------------------------------------------\n")
                st = "Rank: %2d  Name: %-6s  Prop. of words that are special: %f   Num of special words: %3d" %(character.rank, character.name, character.per_special, len(character.special_words))
                print(st)
                output.write(st)
                print()
                output.write("\n\n")
...
def main():
    corpus = pd.read_csv("inaug_speeches.csv", delimiter=",", encoding='latin1')
    character_col = 1
    line_col = 4
    rownum = 1
